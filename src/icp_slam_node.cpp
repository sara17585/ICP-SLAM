...
ICPSlamNode::ICPSlamNode() : local_nh_("~")
{
  ...
  local_nh_.param<double>("max_keyframes_distance", max_keyframes_distance, 0.02); //0.02
...
void ICPSlamNode::laserCallback(const sensor_msgs::LaserScanConstPtr &laser_msg)
{
  ...
  tf::Transform temp_odom = tf_odom_laser;
  tf::Transform temp_map = tf_map_laser;
  tf::Transform temp_map_odom = temp_map * temp_odom.inverse();

  tf::StampedTransform tf_map_odom;
  tf_map_odom.frame_id_ = map_frame_id_;
  tf_map_odom.child_frame_id_ = odom_frame_id_;
  tf_map_odom.stamp_ = ros::Time::now();
  tf_map_odom.setOrigin(temp_map_odom.getOrigin());
  tf_map_odom.setRotation(temp_map_odom.getRotation());
